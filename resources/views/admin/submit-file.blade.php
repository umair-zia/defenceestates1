                                                                ...
                                                                {{-- @foreach ($files_data as $fD) --}}
                                                                    {{-- @if($file->id == $fD->files_id) --}}
                                                                        <tr>
                                                                            ...
                                                                            <td>{{$file->filename ? $file->filename : 'N/A'}}</td>
                                                                            <td>{{$file->city ? $file->city : 'N/A'}}</td>
                                                                            <td>{{$file->phase ?? 'N/A'}}</td>
                                                                            <td>{{$file->type ?? 'N/A'}}</td>
                                                                            <td>Rs.{{$file->price ? $file->price : 'N/A'}}</td>
                                                                            <td>{{$file->area ? $file->area : 'N/A' }}</td>
                                                                            <td>{{$file->unit ? $file->unit : 'N/A'}}</td>
                                                                            {{-- <!-- <td>{{$file->location}}</td> --> --}}
                                                                            {{-- <td>@mdo</td> --}}
                                                                            <td>
                                                                                <a class="theme-btn btn-style-one" href="{{ route('files.show',[$file->id]) }}" style="padding: 5px 15px">Add</a>
                                                                                <a class="theme-btn btn-style-one ml-1" href="{{ route('files.edit',[$file->id]) }}" style="padding: 5px 15px"> Edit</a>
                                                                                <form class="ml-1"  style="display:inline;color: #FFFFFF" action="{{ route('files.destroy',[$file->id]) }}" method="POST">
                                                                                    @csrf
                                                                                    @method('DELETE')
